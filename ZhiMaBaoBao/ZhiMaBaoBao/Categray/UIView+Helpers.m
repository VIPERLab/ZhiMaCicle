...
{
    self.frameOriginX=maxX-self.frameSizeWidth;
}

-(void) setFrameMaxY:(CGFloat)maxY
{
    self.frameOriginY=maxY-self.frameSizeHeight;
}
...
+ (UIView *)createOnePXHeightViewWithFrame:(CGRect)frame bgColor:(UIColor *)color
{
    //为了在retina屏中创建0.5个点的边框，非retina屏要 * 2 ,因为0.5在此屏中创建不出来
    CGFloat calculateHeight = 0.5;
