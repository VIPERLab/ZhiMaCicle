...
- (BOOL)saveMessage:(LGMessage *)message toConverseID:(NSString *)converseID {
    ...
    [messageQueue inDatabase:^(FMDatabase *db) {
        ...
        BOOL successFul = [db executeUpdate:opeartionStr2,converseID,message.msgid,@(message.conversionType),@(message.type),message.fromUid,message.toUidOrGroupId,message.subject,message.text,@(message.sendStatus), @(message.is_read),message.link,message.holderImageUrlString,@(message.isDownLoad),message.videoDownloadUrl,@(message.timeStamp)];
        if (successFul) {
            NSLog(@"插入消息成功");
        } else {
            NSLog(@"插入消息失败");
