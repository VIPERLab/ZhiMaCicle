...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= self.kDNImageFlowMaxSeletedNumber) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:[NSString stringWithFormat:@"不能超过%d张图片",self.kDNImageFlowMaxSeletedNumber] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
