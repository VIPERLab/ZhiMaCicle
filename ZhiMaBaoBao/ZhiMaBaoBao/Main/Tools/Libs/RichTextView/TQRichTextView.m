...
+ (CGFloat)getRechTextViewHeightWithText:(NSString *)text
                               viewWidth:(CGFloat)width
                                    font:(UIFont *)font
lineSpacing:(CGFloat)lineSpacing realWidth:(float*)  realWidth
{
    ...
    //要绘制的文本
    NSString *analyzeText = [TQRichTextView analyzeText:text textRunsArray:richTextRunsArray font:font];
...
goto check;
        ...
        if(realWidth)
        {
            *realWidth=lastRunWidth + lastRunPointX;
        }
    }
...
+ (CGSize)getRechTextViewHeightWithText:(NSString *)text
                              viewWidth:(CGFloat)width
                                   font:(UIFont *)font
                            lineSpacing:(CGFloat)lineSpacing
{
    ...
    NSMutableAttributedString *__attString = [[NSMutableAttributedString alloc] initWithString:analyzeText];
    
    [__attString addAttribute:(NSString*)kCTFontAttributeName value:(__bridge id)aFont range:NSMakeRange(0,__attString.length)];
    CFRelease(aFont);
    
    //文本处理
    for (TQRichTextBaseRun *textRun in richTextRunsArray)
    {
        [textRun replaceTextWithAttributedString:__attString];
    }
    
    
    int lineCount = 0;
    CFRange lineRange = CFRangeMake(0,0);
    CTTypesetterRef typeSetter = CTTypesetterCreateWithAttributedString((__bridge CFAttributedStringRef)__attString);
    float drawLineX = 0;
    float drawLineY = 0;
    BOOL drawFlag = YES;
    ...
    check:  lineRange = CFRangeMake(lineRange.location,testLineLength);
        ...
        if(lineRange.location + lineRange.length >= __attString.length)
        {
            drawFlag = NO;
        }
    ...
    CGSize size = CGSizeMake(realWidth, drawLineY);
    return size;
}
